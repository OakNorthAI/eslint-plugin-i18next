...
const { isTransTag, isUpperCase, isAllowedDOMAttr } = require('../helper');
...
module.exports = {
  ...
  create: function(context) {
    ...
    function getNearestAncestor(node, type, callback) {
      let temp = node.parent;
      while (temp) {
        if (temp.type === type && (!callback || callback(temp))) {
          return temp;
        }
        temp = temp.parent;
      }
    ...
    function isTrans(node) {
      return isTransTag(node.openingElement.name.name);
    }
    ...
    const scriptVisitor = {
      ...
      JSXText(node) {
        ...
        const transAncestor = getNearestAncestor(node, 'JSXElement', isTrans);
        visited.add(node);

        if (!trimed || match(trimed) || transAncestor) {
          return;
        }
